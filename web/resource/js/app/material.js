define(['jquery', 'underscore', 'bootstrap', 'jquery.wookmark', 'jquery.jplayer'], function($, _){
	var material = {
		...
		'init' : function(callback, options) {
			...
			if (!$(this).data('init')) {
				if($this.options.type) {
					...
					$this.modalobj.find('.modal-header .nav li:first-child a').trigger('click');
				}
			}
		...
		'localPage' : function(type, page) {
			...
			$.getJSON('./index.php?c=utility&a=material&do=list&type=' + type, {'page': page}, function(data){
				data = data.message;
				$this.modalobj.find('#material-list-pager .pager').html('');
				if(!_.isEmpty(data.items)) {
					...
					$content.find('.conpon-list').click(function(){
		...
		'buildHtml' : function() {
			...
			dialog['mainDialog'] = '<div id="material-Modal" class="modal fade" tabindex="-1" role="dialog" aria-hidden="true">\n' +
				...
				'						<li role="presentation" class="news ' + (this.options.ignore.news ? 'hide' : '') + '">'+
				'							<a data-toggle="tab" data-type="news" role="tab" aria-controls="news" href="#news">图文</a>'+
				'						</li>'+
				'						<li role="presentation" class="image ' + (this.options.ignore.image ? 'hide' : '') + '">'+
				'							<a data-toggle="tab" data-type="image" role="tab" aria-controls="image" href="#image">图片</a>'+
				'						</li>'+
				'						<li role="presentation" class="voice ' + (this.options.ignore.voice ? 'hide' : '') + '">'+
				'							<a data-toggle="tab" data-type="voice" role="tab" aria-controls="voice" href="#voice">语音</a>'+
				'						</li>'+
				'						<li role="presentation" class="video ' + (this.options.ignore.video ? 'hide' : '') + '">'+
