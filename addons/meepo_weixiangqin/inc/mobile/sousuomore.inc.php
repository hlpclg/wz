...
$stores    = pdo_fetchall("SELECT * FROM " . $tablename . " WHERE nickname like '%{$keword}%' AND  weid = :weid  AND yingcang=1 AND isshow=:isshow     {$condition} ORDER BY love DESC,id DESC,time DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize, array(
    ':weid' => $weid,
    ':isshow' => $isshow
));
