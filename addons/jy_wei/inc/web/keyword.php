...
if($op=='display'){
	...
	include $this->template('keyword_display');
}
...
if($op=='set'){
	...
	if(checksubmit()){
		...
		// 删了再插入
		if($unicode){
			$result = pdo_delete('jy_wei_keyword',array('unicode'=>$unicode,'uniacid'=>$_W['uniacid']));
		}
		foreach($companyid as $key=>$value){
			$data['companyid'] = $value;
			$result = pdo_insert('jy_wei_keyword',$data);
		}
	...
	include $this->template('keyword_set');
}
...
if($op=='delete'){
	...
	$result = pdo_delete('jy_wei_keyword',array('unicode'=>$unicode,'uniacid'=>$_W['uniacid']));
	if($result){
		message("删除成功",$this->createWebUrl('keyword'),"success");
	}else{
		message("删除失败",$this->createWebUrl('keyword'),"error");
	}
}
?>