...
if($data=='branch'){
	...
	$list = pdo_fetchall("SELECT * FROM ".tablename('stonefish_branch_doings')."  WHERE rid = :rid and uniacid=:uniacid and module=:module ".$where." ORDER BY id DESC" , array(':rid' => $rid,':uniacid'=>$_W['uniacid'],':module'=>'stonefish_chailihe'));
    //查询区域以及商家 以及中奖情况
	foreach ($list as &$row) {
		...
		$fid = pdo_fetchcolumn("SELECT from_user FROM " . tablename('stonefish_chailihe_fans') . "  WHERE mobile = :mobile and rid=:rid and uniacid=:uniacid", array(':mobile' => $row['mobile'],':rid' => $rid,':uniacid' => $_W['uniacid']));
		$row['awardinfo']='';
		$awards = pdo_fetchall("SELECT prizeid FROM " . tablename('stonefish_chailihe_fansaward') . " WHERE rid = :rid and from_user=:from_user", array(':rid' => $rid,':from_user' => $fid));
		if(!empty($awards)){
			foreach ($awards as &$awardid) {
				$row['prizename'] = pdo_fetchcolumn("SELECT prizename FROM " . tablename('stonefish_chailihe_prize') . "  WHERE id = :id", array(':id' => $awardid['prizeid']));
	...
	$list = pdo_fetchall("SELECT * FROM ".tablename('stonefish_chailihe_fans')."  WHERE rid = :rid and uniacid=:uniacid ".$where." ORDER BY id DESC" , array(':rid' => $rid,':uniacid'=>$_W['uniacid']));
	//变换变量
	...
	$statustitle.='排名';
	$list = pdo_fetchall("SELECT * FROM ".tablename('stonefish_chailihe_fans')."  WHERE rid = :rid and uniacid=:uniacid ORDER BY ".$ORDER." DESC,id asc" , array(':rid' => $rid,':uniacid'=>$_W['uniacid']));
	//变换变量
	foreach ($list as &$lists) {
		$lists['status']='';
		if($lists['zhongjiang']==0){
    ...
    $list = pdo_fetchall("SELECT a.*,b.realname,b.mobile,b.qq,b.email,b.address,b.gender,b.telephone,b.idcard,b.company,b.occupation,b.position FROM ".tablename('stonefish_chailihe_fansaward')." as a,".tablename('stonefish_chailihe_fans')." as b WHERE a.rid=b.rid and a.uniacid=b.uniacid and a.from_user=b.from_user and a.rid = :rid and a.uniacid=:uniacid ".$params." ORDER BY a.id DESC", array(':rid' => $rid,':uniacid'=>$_W['uniacid']));
    foreach ($list as &$lists) {
	    $lists['status']='';
		if($lists['zhongjiang']==0){
		    $lists['status']='未中奖';
