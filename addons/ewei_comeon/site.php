...
class Ewei_comeonModuleSite extends WeModuleSite {
    ...
    public function get_reply($rid) {
        ...
        return pdo_fetch("SELECT * FROM " . tablename('ewei_comeon_reply') . " WHERE rid = :rid limit 1", array(':rid' => $rid));
    }
    ...
    public function get_awards($rid) {
        ...
        return pdo_fetchall("SELECT * FROM " . tablename('ewei_comeon_award') . " WHERE rid = :rid order by point asc", array(':rid' => $rid));
    }
    ...
    public function get_rules($rid) {
        ...
        return pdo_fetchall("SELECT * FROM " . tablename('ewei_comeon_rule') . " WHERE rid = :rid order by point desc", array(':rid' => $rid));
    }
    ...
    public function get_sysset() {
        ...
        return pdo_fetch("SELECT * FROM " . tablename('ewei_comeon_sysset') . " WHERE weid = :weid limit 1", array(':weid' => $this->weid));
    }
    ...
    public function doWebManage() {
        ...
        if (!empty($list)) {
            foreach ($list as &$item) {
                ...
                $comeon = pdo_fetch("SELECT viewnum,starttime,endtime,isshow FROM " . tablename('ewei_comeon_reply') . " WHERE rid = :rid ", array(':rid' => $item['id']));
                $item['fansnum'] = pdo_fetchcolumn("select count(*) from " . tablename('ewei_comeon_fans') . " where rid=:rid ", array(":rid" => $item['id']));
    ...
    public function doWebSysset() {
        ...
        if (checksubmit('submit')) {
            ...
            if (!empty($set)) {
                pdo_update('ewei_comeon_sysset', $data, array('id' => $set['id']));
            } else {
                pdo_insert('ewei_comeon_sysset', $data);
            }
    ...
    private function get_code($id,$appid, $fansid){
         ...
         global $_W;
         $url = $_W['siteroot'] . "app/index.php?i=".$_W['uniacid']."&c=entry&m=ewei_comeon&do=index&id={$id}".(!empty($fansid)?'&fansid='.$fansid:"");
    ...
    public function doMobileHelp() {
        ...
        if (!empty($openid)) {
            ...
            if ($reply['other_times'] > 0) {
                //检测助力别人总限制
                $count = pdo_fetchcolumn("select count(*) from " . tablename('ewei_comeon_fans_help') . " where rid=:rid and from_user=:from_user  ", array(":rid" => $rid, ":from_user" => $openid));
            ...
            if ($reply['other_day_times'] > 0) {
                //检测每天助力限制
                $count = pdo_fetchcolumn("select count(*) from " . tablename('ewei_comeon_fans_help') . " where rid=:rid and from_user=:from_user and date=:date  ", array(":rid" => $rid, ":from_user" => $openid, ":date" => $today));
            ...
            if ($reply['other_one_times'] > 0) {
                //检测助力别人总限制
                $count = pdo_fetchcolumn("select count(*) from " . tablename('ewei_comeon_fans_help') . " where rid=:rid and fansid=:fansid and from_user=:from_user  ", array(":rid" => $rid, ":fansid" => $fansid, ":from_user" => $openid));
            ...
            if ($reply['other_one_day_times'] > 0) {
                //检测每天助力限制
                $count = pdo_fetchcolumn("select count(*) from " . tablename('ewei_comeon_fans_help') . " where rid=:rid and fansid=:fansid and from_user=:from_user and date=:date  ", array(":rid" => $rid, ":fansid" => $fansid, ":from_user" => $openid, ":date" => $today));
            ...
            if ($reply['other_times'] > 0) {
                //检测助力别人总限制
                $key = 'ewei_comeon_other_times_' . $rid;
            ...
            if ($reply['other_day_times'] > 0) {
                //检测每天助力限制
                $key = 'ewei_comeon_other_day_times_' . $today . "_" . $rid;
                $count = intval($_GPC[$key]);
                if ($count >= $reply['other_day_times']) {
                    die(json_encode(array('result' => 0, "msg" => "今天您已经给别人助力 {$count} 次啦，不能再给 TA 助力啦!")));
                }
            ...
            if ($reply['other_one_times'] > 0) {
                //检测助力相同人总限制
                $key = 'ewei_comeon_other_one_times_' . $fansid . "_" . $rid;
                $count = intval($_GPC[$key]);
                if ($count >= $reply['other_one_times']) {
                    die(json_encode(array('result' => 0, "msg" => "您已经给 TA 助力 {$count} 次啦，不能再给 TA 助力啦!")));
                }
            ...
            if ($reply['other_one_day_times'] > 0) {
                //检测每天助力相同人限制
                $key = 'ewei_comeon_other_one_day_times_' . $fansid . '_' . $today . "_" . $rid;
                $count = intval($_GPC[$key]);
                if ($count >= $reply['other_one_day_times']) {
                    die(json_encode(array('result' => 0, "msg" => "今天您已经给 TA 助力 {$count} 次啦，不能再给 TA 助力啦!")));
                }
