...
function write_cache($filename, $data) {
    ...
    public function fieldsFormSubmit($rid) {
        ...
        //基础数据缓存
        $insert['id'] = $id;
        $d = pdo_fetch("select * from ".tablename('ewei_comeon_reply')." where rid=:rid limit 1",array(":rid"=>$rid));
        ...
        //规则缓存
        $rules = pdo_fetchall("select * from ".tablename('ewei_comeon_rule')." where rid=:rid order by point desc",array(":rid"=>$rid));
