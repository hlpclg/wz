...
//=========步骤3：使用jsapi调起支付============
	...
	public function  doMobileAjax()
	{
		...
		if ($action == 'kanjia') {//砍价
			...
			$gmCount = $this->getOrderCount($kid, $this::$KJ_STATUS_GM);
			$wkj = DBUtil::findById(DBUtil::$TABLE_WKJ, $kid);
	...
	public function doMobileRanking()
	{
		...
		if ($wkj['pay_type'] == 1) {//立即支付
			$leftCount = $wkj['p_kc'] - $this->getOrderCount($kid, $this::$KJ_STATUS_GM);
		}

		if ($wkj['pay_type'] == 2) {//货到付款
			$leftCount = $wkj['p_kc'] - $this->getOrderCount($kid, $this::$KJ_STATUS_XD);
		}
	...
	{
		$orderCount = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename(DBUtil::$TABLE_WJK_ORDER) . " WHERE kid =:kid and status=:status", array(":kid" => $kid, ":status" => $status));
		return $orderCount;
	}
