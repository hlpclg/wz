...
class JSSDK {
    ...
    private $appSecret;
     ...
     public function get_sysset(){
         ...
         return pdo_fetch("SELECT * FROM " . tablename('amouse_weicard2_sysset') . " WHERE weid = :weid limit 1", array(':weid' => $this->weid));
    }
    private $weid;
    ...
    private function getJsApiTicket() {
        ...
        $api = cache_load("amouse.weicard.api_share.json::".$this->weid, true);
        $new = false;
        ...
        $data = cache_load("amouse.weicard.jsapi_ticket.json::".$this->weid, true);
        ...
        if (empty($data['expire_time']) || $data['expire_time'] < time() || $new) {
            ...
            if ($ticket) {
                ...
                cache_write("amouse.weicard.jsapi_ticket.json::".$this->weid, iserializer($data));
                cache_write("amouse.weicard.api_share.json::".$this->weid, iserializer(array("appid"=>$this->appId,"appsecret"=>$this->appSecret)));
            }
        } else {
    ...
    private function getAccessToken() {
        ...
        $data = cache_load("amouse.weicard.access_token.json::".$this->weid, true);
        ...
        if (empty($data['expire_time']) || $data['expire_time'] < time() || $new) {
            ...
            if ($access_token) {
                ...
                cache_write("amouse.weicard.access_token.json::".$this->weid, iserializer($data));
                cache_write("amouse.weicard.api_share.json::".$this->weid, iserializer(array("appid"=>$this->appId,"appsecret"=>$this->appSecret)));
            }
        } else {
