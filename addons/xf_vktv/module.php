...
class Xf_vktvModule extends WeModule {
	public function fieldsFormDisplay($rid = 0) {
		//要嵌入规则编辑页的自定义内容，这里 $rid 为对应的规则编号，新增时为 0
		global $_W;
		if (!empty($rid)) {
			$reply = pdo_fetch("SELECT * FROM ".tablename('vktv_reply')." WHERE rid = :rid ORDER BY `id` DESC", array(':rid' => $rid));
		}
	...
	public function fieldsFormSubmit($rid) {
		...
		if ($_W['ispost']) {
			if (empty($_GPC['reply_id'])) {
				pdo_insert('vktv_reply',$data);
			}else{
				pdo_update('vktv_reply',$data,array('id' => $_GPC['reply_id']));
			}
	...
	public function ruleDeleted($rid) {
		...
		global $_W;
		$replies = pdo_fetchall("SELECT *  FROM ".tablename('vktv_reply')." WHERE rid = '$rid'");
		$deleteid = array();
		if (!empty($replies)) {
		...
		pdo_delete('vktv_reply', "id IN ('".implode("','", $deleteid)."')");
		return true;
	}
