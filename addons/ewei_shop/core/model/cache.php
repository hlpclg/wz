...
class Ewei_DShop_Cache
{
    function get_key($key = '', $uniacid = '')
    {
        ...
        return EWEI_SHOP_PREFIX . '_' . $GLOBALS['_W']['config']['setting']['authkey'] . '_' . $uniacid . '_' . $key;
    }
    ...
    function file_read($key = '', $isArray = true, $uniacid = '')
    {
        ...
        $content = @file_get_contents(IA_ROOT . "/addons/ewei_shop/data/cache/" . $uniacid . "/" . $key);
        if (empty($content)) {
            return false;
        }
    ...
    function file_set($key = '', $value = null, $uniacid = '')
    {
        ...
        $path    = IA_ROOT . "/addons/ewei_shop/data/cache/" . $uniacid;
        if (!is_dir($path)) {
            load()->func('file');
            @mkdirs($path);
        }
