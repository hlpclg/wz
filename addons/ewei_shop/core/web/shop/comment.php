...
if ($operation == 'display') {
    ...
    pdo_update('ewei_shop_order_comment', array(
        'deleted' => 1
    ), array(
        'id' => $id,
        'uniacid' => $_W['uniacid']
    ));
    $goods = pdo_fetch('select id,thumb,title from ' . tablename('ewei_shop_goods') . ' where id=:id and uniacid=:uniacid limit 1', array(
        ':id' => $item['goodsid'],
        ':uniacid' => $_W['uniacid']
    ));
    ...
    ca('shop.comment.add');
    $id      = intval($_GPC['id']);
    $item    = pdo_fetch("SELECT * FROM " . tablename('ewei_shop_order_comment') . " WHERE id =:id and uniacid=:uniacid limit 1 ", array(
        ':id' => $id,
        ':uniacid' => $_W['uniacid']
    ));
    ...
    if (checksubmit()) {
        ...
        if (!empty($id)) {
            pdo_update('ewei_shop_order_comment', $data, array(
                'id' => $id
            ));
    ...
    $order = pdo_fetch('select id,ordersn from ' . tablename('ewei_shop_order') . ' where id=:id and uniacid=:uniacid limit 1', array(
        ':id' => $item['orderid'],
        ':uniacid' => $_W['uniacid']
    ));
    if (checksubmit()) {
