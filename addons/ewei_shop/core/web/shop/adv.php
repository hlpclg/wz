...
if ($operation == 'display') {
    ...
    if (!empty($_GPC['displayorder'])) {
        ...
        foreach ($_GPC['displayorder'] as $id => $displayorder) {
            pdo_update('ewei_shop_adv', array(
    ...
    if (checksubmit('submit')) {
        ...
        if (!empty($id)) {
            pdo_update('ewei_shop_adv', $data, array(
                'id' => $id
            ));
    ...
    $item = pdo_fetch("select * from " . tablename('ewei_shop_adv') . " where id=:id and uniacid=:uniacid limit 1", array(
        ":id" => $id,
        ":uniacid" => $_W['uniacid']
    ));
    ...
    pdo_delete('ewei_shop_adv', array(
        'id' => $id
    ));
