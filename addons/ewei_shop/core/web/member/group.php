...
if ($operation == 'display') {
    ...
    $list    = array(
        array(
            'groupname' => '无分组',
            'membercount' => pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_member') . ' where uniacid=:uniacid and groupid=0 limit 1', array(
                ':uniacid' => $_W['uniacid']
            ))
        )
    );
    ...
    foreach ($alllist as &$row) {
        $row['membercount'] = pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_member') . ' where uniacid=:uniacid and groupid=:groupid limit 1', array(
            ':uniacid' => $_W['uniacid'],
            ':groupid' => $row['id']
        ));
    }
    ...
    if (checksubmit('submit')) {
        ...
        if (!empty($id)) {
            pdo_update('ewei_shop_member_group', $data, array(
                'id' => $id,
                'uniacid' => $_W['uniacid']
            ));
    ...
    pdo_delete('ewei_shop_member_group', array(
        'id' => $id,
        'uniacid' => $_W['uniacid']
    ));
