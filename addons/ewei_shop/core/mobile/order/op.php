...
if ($_W['isajax']) {
	if ($operation == 'cancel') {
		...
		$orderid = intval($_GPC['orderid']);
		$order = pdo_fetch('select id,status,openid,couponid from ' . tablename('ewei_shop_order') . ' where id=:id and uniacid=:uniacid and openid=:openid limit 1', array(':id' => $orderid, ':uniacid' => $uniacid, ':openid' => $openid));
		if (empty($order)) {
			show_json(0, '订单未找到!');
		}
		if ($order['status'] != 2) {
		...
		$orderid = intval($_GPC['orderid']);
		$order = pdo_fetch('select id,status,price,refundid,goodsprice,dispatchprice,deductprice,deductcredit2,finishtime,isverify,virtual from ' . tablename('ewei_shop_order') . ' where id=:id and uniacid=:uniacid and openid=:openid limit 1', array(':id' => $orderid, ':uniacid' => $uniacid, ':openid' => $openid));
		if (empty($order)) {
			show_json(0, '订单未找到!');
		}
		...
		pdo_update('ewei_shop_order', array('userdeleted' => 1), array('id' => $order['id']));
		show_json(1);
	}
}
if ($operation == 'refund') {
