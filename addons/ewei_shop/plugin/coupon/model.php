...
if (!class_exists('CouponModel')) {
	class CouponModel extends PluginModel
	{
		...
		{
			global $_W;
			$_var_1 = pdo_fetch('SELECT id,total FROM ' . tablename('ewei_shop_coupon') . ' WHERE id=:id and uniacid=:uniacid ', array(':id' => $_var_0, ':uniacid' => $_W['uniacid']));
			if (empty($_var_1)) {
				return 0;
			}
			if ($_var_1['total'] == -1) {
			...
			$_var_2 = pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_coupon_data') . ' where couponid=:couponid and uniacid=:uniacid limit 1', array(':couponid' => $_var_0, ':uniacid' => $_W['uniacid']));
		...
		function creditshop($_var_3 = 0)
		{
			...
			$_var_5 = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_creditshop_log') . ' WHERE `id`=:id and `uniacid`=:uniacid  limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $_var_3));
			if (!empty($_var_5)) {
				...
				pdo_insert('ewei_shop_coupon_log', $_var_8);
				$_var_9 = array('uniacid' => $_W['uniacid'], 'openid' => $_var_5['openid'], 'couponid' => $_var_5['couponid'], 'gettype' => 2, 'gettime' => time());
				pdo_insert('ewei_shop_coupon_data', $_var_9);
				$_var_1 = pdo_fetch('select * from ' . tablename('ewei_shop_coupon') . ' where id=:id limit 1', array(':id' => $_var_5['couponid']));
				$_var_1 = $this->setCoupon($_var_1, time());
				$_var_10 = $this->getSet();
				$this->sendMessage($_var_1, 1, $_var_6, $_var_10['templateid']);
				pdo_update('ewei_shop_creditshop_log', array('status' => 3), array('id' => $_var_3));
			}
		}
		...
		function payResult($_var_14)
		{
			...
			$_var_5 = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_coupon_log') . ' WHERE `logno`=:logno and `uniacid`=:uniacid  limit 1', array(':uniacid' => $_W['uniacid'], ':logno' => $_var_14));
			if (empty($_var_5)) {
			...
			if (empty($_var_5['status'])) {
				...
				$_var_15['status'] = 1;
				pdo_update('ewei_shop_coupon_log', $_var_15, array('id' => $_var_5['id']));
			...
			$_var_16 = $_W['siteroot'] . 'app/index.php?i=' . $_W['uniacid'] . '&c=entry&m=ewei_shop&do=member';
			if ($_var_1['coupontype'] == 0) {
				$_var_16 = $_W['siteroot'] . 'app/index.php?i=' . $_W['uniacid'] . '&c=entry&m=ewei_shop&do=shop&p=list';
			} else {
				$_var_16 = $_W['siteroot'] . 'app/index.php?i=' . $_W['uniacid'] . '&c=entry&m=ewei_shop&do=member&p=recharge';
			}
			if (strexists($_var_16, '/addons/ewei_shop/plugin/coupon/core/mobile/')) {
				$_var_16 = str_replace('/addons/ewei_shop/plugin/coupon/core/mobile/', '/', $_var_16);
			}
			if (strexists($_var_16, '/addons/ewei_shop/')) {
				$_var_16 = str_replace('/addons/ewei_shop/', '/', $_var_16);
			}
		...
		function sendMessage($_var_1, $_var_17, $_var_6, $_var_18 = '', $_var_19 = null)
		{
			...
			$_var_16 = empty($_var_1['respurl']) ? $_W['siteroot'] . 'app/index.php?i=' . $_W['uniacid'] . '&c=entry&m=ewei_shop&do=plugin&p=coupon&method=my' : $_var_1['respurl'];
		...
		function sendReturnMessage($_var_25, $_var_1)
		{
			...
			$_var_16 = $_W['siteroot'] . 'app/index.php?i=' . $_W['uniacid'] . '&c=entry&m=ewei_shop&do=plugin&p=coupon&method=my';
			if (!empty($_var_18)) {
				m('message')->sendTplNotice($_var_25, $_var_18, $_var_24, $_var_16);
			} else {
				m('message')->sendCustomNotice($_var_25, $_var_24, $_var_16);
		...
		function useRechargeCoupon($_var_5)
		{
			...
			$_var_9 = pdo_fetch('select id,openid,couponid,used from ' . tablename('ewei_shop_coupon_data') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $_var_5['couponid'], ':uniacid' => $_W['uniacid']));
			if (empty($_var_9)) {
				return;
			}
			...
			$_var_1 = pdo_fetch('select enough,backcredit,backmoney,backredpack from ' . tablename('ewei_shop_coupon') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $_var_9['couponid'], ':uniacid' => $_W['uniacid']));
			if (empty($_var_1)) {
				return;
			}
			...
			pdo_update('ewei_shop_coupon_data', array('used' => 1, 'usetime' => time(), 'ordersn' => $_var_5['logno']), array('id' => $_var_9['id']));
		...
		function useConsumeCoupon($_var_35 = 0)
		{
			...
			$_var_36 = pdo_fetch('select ordersn,createtime,couponid from ' . tablename('ewei_shop_order') . ' where id=:id and status>=0 and uniacid=:uniacid limit 1', array(':id' => $_var_35, ':uniacid' => $_W['uniacid']));
			if (empty($_var_36)) {
				return;
			}
			$_var_1 = false;
			...
			pdo_update('ewei_shop_coupon_data', array('used' => 1, 'usetime' => $_var_36['createtime'], 'ordersn' => $_var_36['ordersn']), array('id' => $_var_36['couponid']));
		}
		...
		function returnConsumeCoupon($_var_36)
		{
			global $_W;
			if (!is_array($_var_36)) {
				$_var_36 = pdo_fetch('select id,openid,ordersn,createtime,couponid,status,finishtime from ' . tablename('ewei_shop_order') . ' where id=:id and status>=0 and uniacid=:uniacid limit 1', array(':id' => intval($_var_36), ':uniacid' => $_W['uniacid']));
			}
			if (empty($_var_36)) {
				return;
			}
			...
			if (!empty($_var_1['returntype'])) {
				if (!empty($_var_1['used'])) {
					pdo_update('ewei_shop_coupon_data', array('used' => 0, 'usetime' => 0, 'ordersn' => ''), array('id' => $_var_36['couponid']));
		...
		function backConsumeCoupon($_var_36)
		{
			global $_W;
			if (!is_array($_var_36)) {
				$_var_36 = pdo_fetch('select id,openid,ordersn,createtime,couponid,status,finishtime,virtual from ' . tablename('ewei_shop_order') . ' where id=:id and status>=0 and uniacid=:uniacid limit 1', array(':id' => intval($_var_36), ':uniacid' => $_W['uniacid']));
			}
			if (empty($_var_36)) {
				return;
			}
			...
			if ($_var_37) {
				$_var_41 = pdo_fetchcolumn('select ifnull( sum(og.realprice),0) from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join ' . tablename('ewei_shop_order') . ' o on o.id=og.orderid ' . ' where o.id=:orderid and o.openid=:openid and o.uniacid=:uniacid ', array(':uniacid' => $_W['uniacid'], ':openid' => $_var_36['openid'], ':orderid' => $_var_36['id']));
				$_var_29 = $_var_1['backcredit'];
				if (!empty($_var_29)) {
					if (strexists($_var_29, '%')) {
				...
				pdo_update('ewei_shop_coupon_data', array('back' => 1, 'backtime' => time()), array('id' => $_var_36['couponid']));
		...
		{
			global $_W;
			return pdo_fetch('select * from ' . tablename('ewei_shop_coupon') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $_var_0, ':uniacid' => $_W['uniacid']));
		}

		function getCouponByDataID($_var_42 = 0)
		{
			global $_W;
			$_var_9 = pdo_fetch('select id,openid,couponid,used,back,backtime from ' . tablename('ewei_shop_coupon_data') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $_var_42, ':uniacid' => $_W['uniacid']));
			if (empty($_var_9)) {
				return false;
			}
			$_var_1 = pdo_fetch('select * from ' . tablename('ewei_shop_coupon') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $_var_9['couponid'], ':uniacid' => $_W['uniacid']));
			if (empty($_var_1)) {
				return false;
			}
		...
		function setCoupon($_var_43, $_var_33, $_var_44 = true)
		{
			...
			if ($_var_44) {
				...
				$_var_43['canget'] = true;
				if ($_var_43['getmax'] > 0) {
					$_var_45 = pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_coupon_data') . ' where couponid=:couponid and openid=:openid and uniacid=:uniacid and gettype=1 limit 1', array(':couponid' => $_var_43['id'], ':openid' => $_var_25, ':uniacid' => $_W['uniacid']));
		...
		function setShare()
		{
			...
			$_var_10 = $this->getSet();
			$_var_25 = m('user')->getOpenid();
			$_var_16 = $_W['siteroot'] . "app/index.php?i={$_W['uniacid']}&c=entry&p=coupon&m=ewei_shop&do=plugin";
