...
if ($operation == 'temp') {
    ...
    foreach ($datas as $d) {
        $olddata = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_virtual_data') . ' WHERE pvalue=:pvalue and typeid=:typeid and uniacid=:uniacid ', array(
            ':pvalue' => $d['pvalue'],
            ':typeid' => $_GPC['typeid'],
            ':uniacid' => $_W['uniacid']
        ));
        if (empty($olddata)) {
            ...
            if (empty($olddata['openid'])) {
                pdo_update('ewei_shop_virtual_data', $d, array(
                    'id' => $olddata['id']
                ));
            } else {
                $noinsert .= $d['pvalue'] . ',';
