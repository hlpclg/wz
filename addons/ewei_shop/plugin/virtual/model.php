...
if (!class_exists('VirtualModel')) {
    class VirtualModel extends PluginModel
    {
        ...
        {
            global $_W, $_GPC;
            $goods = pdo_fetch('select virtual from ' . tablename('ewei_shop_goods') . ' where id=:id and type=3 and uniacid=:uniacid limit 1', array(
                ':id' => $id,
                ':uniacid' => $_W['uniacid']
            ));
            if (empty($goods)) {
            ...
            if (!empty($goods['virtual'])) {
                $stock = pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_virtual_data') . " where typeid=:typeid and uniacid=:uniacid and openid='' limit 1", array(
                ...
                foreach ($alloptions as $opt) {
                    ...
                    $c = pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_virtual_data') . " where typeid=:typeid and uniacid=:uniacid and openid='' limit 1", array(
                        ':typeid' => $opt['virtual'],
                        ':uniacid' => $_W['uniacid']
                    ));
        ...
        public function updateStock($typeid = 0)
        {
            global $_W;
            $goodsids = array();
            $goods    = pdo_fetchall('select id from ' . tablename('ewei_shop_goods') . ' where type=3 and virtual=:virtual and uniacid=:uniacid limit 1', array(
                ':virtual' => $typeid,
                ':uniacid' => $_W['uniacid']
            ));
            foreach ($goods as $g) {
            ...
            $alloptions = pdo_fetchall("select id, goodsid from " . tablename('ewei_shop_goods_option') . " where virtual=:virtual and uniacid=:uniacid", array(
                ':uniacid' => $_W['uniacid'],
                ':virtual' => $typeid
            ));
            foreach ($alloptions as $opt) {
        ...
        {
            global $_W, $_GPC;
            $goods        = pdo_fetch('select id,goodsid,total,realprice from ' . tablename('ewei_shop_order_goods') . ' where  orderid=:orderid and uniacid=:uniacid limit 1', array(
                ':uniacid' => $_W['uniacid'],
                ':orderid' => $order['id']
            ));
            $g            = pdo_fetch('select id,credit,sales,salesreal from ' . tablename('ewei_shop_goods') . ' where  id=:id and uniacid=:uniacid limit 1', array(
                ':uniacid' => $_W['uniacid'],
                ':id' => $goods['goodsid']
            ));
            $virtual_data = pdo_fetchall('SELECT id,typeid,fields FROM ' . tablename('ewei_shop_virtual_data') . ' WHERE typeid=:typeid and openid=:openid and uniacid=:uniacid order by rand() limit ' . $goods['total'], array(
                ':openid' => '',
                ':typeid' => $order['virtual'],
                ':uniacid' => $_W['uniacid']
            ));
            $type         = pdo_fetch('select fields from ' . tablename('ewei_shop_virtual_type') . ' where id=:id and uniacid=:uniacid limit 1 ', array(
                ':id' => $order['virtual'],
                ':uniacid' => $_W['uniacid']
            ));
            ...
            foreach ($virtual_data as $vd) {
                ...
                pdo_update('ewei_shop_virtual_type', 'usedata=usedata+1', array(
                    'id' => $vd['typeid']
                ));
