...
if ($operation == 'display') {
    ...
    foreach ($list as &$row) {
        $row['salercount'] = pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_saler') . ' where storeid=:storeid limit 1', array(
            ':storeid' => $row['id']
        ));
    }
    unset($row);
    ...
    $item = pdo_fetch("SELECT * FROM " . tablename('ewei_shop_store') . " WHERE id =:id and uniacid=:uniacid limit 1", array(
        ':uniacid' => $_W['uniacid'],
        ':id' => $id
    ));
    if (checksubmit('submit')) {
        ...
        if (!empty($id)) {
            pdo_update('ewei_shop_store', $data, array(
                'id' => $id,
                'uniacid' => $_W['uniacid']
            ));
    ...
    pdo_delete('ewei_shop_store', array(
        'id' => $id,
        'uniacid' => $_W['uniacid']
    ));
