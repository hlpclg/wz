    ...
    public function respond($obj = null)
    {
        ...
        if ($msgtype == 'text' || $event == 'click') {
            ...
            if (!$obj->inContext) {
                ...
                $order = pdo_fetch('select * from ' . tablename('ewei_shop_order') . ' where verifycode=:verifycode and uniacid=:uniacid  limit 1', array(
                    ':verifycode' => $content,
                    ':uniacid' => $_W['uniacid']
                ));
                if (empty($order)) {
                ...
                $storeids = array();
                $goods    = pdo_fetchall("select og.goodsid,og.price,g.title,g.thumb,og.total,g.credit,og.optionid,g.isverify,g.storeids from " . tablename('ewei_shop_order_goods') . " og " . " left join " . tablename('ewei_shop_goods') . " g on g.id=og.goodsid " . " where og.orderid=:orderid and og.uniacid=:uniacid ", array(
                    ':uniacid' => $_W['uniacid'],
                    ':orderid' => $order['id']
                ));
                foreach ($goods as $g) {
