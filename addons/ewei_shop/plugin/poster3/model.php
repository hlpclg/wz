...
if (!class_exists('PosterModel')) {
    class PosterModel extends PluginModel
    {
        public function checkScan()
        {
            ...
            $poster = pdo_fetch('select id,times from ' . tablename('ewei_shop_poster') . ' where id=:id and uniacid=:uniacid limit 1', array(
                ':uniacid' => $_W['uniacid'],
                ':id' => $posterid
            ));
            if (empty($poster)) {
        ...
        public function scanTime($openid, $from_openid, $poster)
        {
            ...
            global $_W, $_GPC;
            $scancount = pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_poster_scan') . ' where openid=:openid  and posterid=:posterid and uniacid=:uniacid limit 1', array(
                ':openid' => $openid,
                ':posterid' => $poster['id'],
                ':uniacid' => $_W['uniacid']
            ));
        ...
        public function createCommissionPoster($openid, $goodsid = 0)
        {
            ...
            $poster = pdo_fetch('select * from ' . tablename('ewei_shop_poster') . ' where uniacid=:uniacid and type=:type and isdefault=1 limit 1', array(
                ':uniacid' => $_W['uniacid'],
                ':type' => $type
            ));
            if (empty($poster)) {
        ...
        public function getQR($poster, $member, $goodsid = 0)
        {
            ...
            if ($poster['type'] == 1) {
                ...
                $qr    = pdo_fetch('select * from ' . tablename('ewei_shop_poster_qr') . ' where openid=:openid and acid=:acid and type=:type limit 1', array(
                ...
                $qr    = pdo_fetch('select * from ' . tablename('ewei_shop_poster_qr') . ' where openid=:openid and acid=:acid and type=:type and goodsid=:goodsid limit 1', array(
                    ':openid' => $member['openid'],
                    ':acid' => $_W['acid'],
                    ':type' => 3,
                    ':goodsid' => $goodsid
                ...
                $qr          = pdo_fetch('select * from ' . tablename('ewei_shop_poster_qr') . ' where openid=:openid and acid=:acid and type=4 limit 1', array(
                    ':openid' => $member['openid'],
                    ':acid' => $acid
                ));
                if (empty($qr)) {
        ...
        public function getQRBySceneid($sceneid = 0, $ticket = '')
        {
            global $_W;
            if (empty($ticket) || empty($sceneid)) {
                return false;
            }
            $qrs   = pdo_fetchall('select * from ' . tablename('ewei_shop_poster_qr') . ' where ticket=:ticket and acid=:acid and type=4 limit 1', array(
                ':ticket' => $ticket,
                ':acid' => $_W['acid']
            ));
            $count = count($qrs);
            ...
            return pdo_fetch('select * from ' . tablename('ewei_shop_poster_qr') . ' where sceneid=:sceneid and ticket=:ticket and acid=:acid and type=4 limit 1', array(
                ':sceneid' => $sceneid,
                ':ticket' => $ticket,
                ':acid' => $_W['acid']
            ));
        ...
        public function checkMember($openid = '')
        {
            ...
            if (empty($member)) {
                ...
                pdo_update('ewei_shop_member', $member, array(
                    'id' => $member['id']
                ));
                $member['isnew'] = false;
            }
