...
class PosterProcessor extends PluginProcessor
{
    ...
    private function responseText($obj)
    {
        ...
        $resp = ihttp_post($_W['siteroot'] . 'app/index.php?i=' . $_W['uniacid'] . '&c=entry&m=ewei_shop&do=plugin&p=poster&method=build', array(
    ...
    private function responseScan($obj)
    {
        ...
        $qr = $this->model->getQRBySceneid($sceneid, $ticket);
        if (empty($qr)) {
            return $this->responseDefault($obj);
        }
        $poster = pdo_fetch('select * from ' . tablename('ewei_shop_poster') . ' where type=4 and isdefault=1 and uniacid=:uniacid limit 1', array(
            ':uniacid' => $_W['uniacid']
        ));
        if (empty($poster)) {
            return $this->responseDefault($obj);
        ...
        $news = array(
            array(
                'title' => $poster['resptitle'],
                'description' => $poster['respdesc'],
                'picurl' => tomedia($poster['respthumb']),
                'url' => $url
            )
        );
    ...
    private function responseSubscribe($obj)
    {
        ...
        $qrmember = m('member')->getMember($qr['openid']);
        $log      = pdo_fetch('select * from ' . tablename('ewei_shop_poster_log') . ' where openid=:openid and posterid=:posterid and uniacid=:uniacid limit 1', array(
            ':openid' => $openid,
            ':posterid' => $poster['id'],
            ':uniacid' => $_W['uniacid']
        ));
    ...
    private function commission($poster, $member, $qrmember)
    {
        ...
        if ($p) {
            $cset = $p->getSet();
            if (!empty($cset)) {
                if ($member['isagent'] != 1 && empty($member['agentid'])) {
                    if ($qrmember['isagent'] == 1 && $qrmember['status'] == 1) {
                        if (!empty($poster['bedown'])) {
                            ...
                            $p->sendMessage($qrmember['openid'], array(
                            ...
                            if (!empty($poster['beagent'])) {
                                ...
                                $p->sendMessage($member['openid'], array(
                                    'nickname' => $member['nickname'],
                                    'agenttime' => $time
