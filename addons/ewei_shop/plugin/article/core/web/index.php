...
if ($operation == 'display') {
	...
	$psize = 15;
	$articles = pdo_fetchall("SELECT a.id,a.article_title,a.article_category,a.article_keyword,a.article_date,a.article_readnum,a.article_likenum,a.article_state,c.category_name FROM " . tablename('ewei_shop_article') . " a left join " . tablename('ewei_shop_article_category') . " c on c.id=a.article_category  WHERE a.uniacid= :uniacid " . $select_title . $select_category . " order by article_date desc LIMIT " . ($pindex - 1) * $psize . ',' . $psize, array(':uniacid' => $_W['uniacid']));
	...
	if (!empty($aid)) {
		...
		$pindex = max(1, intval($page));
		$psize = 20;
		$article = pdo_fetch("SELECT a.id,a.article_title,a.article_category,a.article_keyword,a.article_date,a.article_readnum,a.article_readnum_v,a.article_likenum,a.article_likenum_v,c.category_name FROM " . tablename('ewei_shop_article') . " a left join " . tablename('ewei_shop_article_category') . " c on c.id=a.article_category  WHERE a.id=:aid and a.uniacid= :uniacid LIMIT 1 ", array(':aid' => $aid, ':uniacid' => $_W['uniacid']));
