...
{
	function get_item_taobao($itemid = '', $taobaourl = '', $pcate = 0, $ccate = 0)
	{
		global $_W;
		$g = pdo_fetch("select * from " . tablename('ewei_dshop_goods') . " where taobaoid=:taobaoid and deleted=0 limit 1", array(":taobaoid" => $itemid));
		if ($g) {
	...
	function save_goods($item = array(), $taobaourl = '')
	{
		...
		$data = array("uniacid" => $_W['uniacid'], "taobaoid" => $item['itemId'], "taobaourl" => $taobaourl, "title" => $item['title'], "total" => $item['total'], "marketprice" => $item['marketprice'], "pcate" => $item['pcate'], "ccate" => $item['ccate'], "sales" => $item['sales'], "createtime" => time(), "updatetime" => time(), 'status' => 0);
		$thumb_url = array();
		$pics = $item['pics'];
		$piclen = count($pics);
		if ($piclen > 0) {
		...
		$goods = pdo_fetch("select * from " . tablename('ewei_dshop_goods') . " where taobaoid=:taobaoid", array(":taobaoid" => $item['itemId']));
		if (empty($goods)) {
			pdo_insert("ewei_dshop_goods", $data);
			$goodsid = pdo_insertid();
		} else {
			$goodsid = $goods['id'];
			unset($data['createtime']);
			pdo_update('ewei_dshop_goods', $data, array("id" => $goodsid));
		}
		$goods_params = pdo_fetchall("select * from " . tablename('ewei_dshop_goods_param') . " where goodsid=:goodsid ", array(":goodsid" => $goodsid));
		$params = $item['params'];
		$paramids = array();
		$displayorder = 0;
		foreach ($params as $p) {
			$oldp = pdo_fetch("select * from " . tablename('ewei_dshop_goods_param') . " where goodsid=:goodsid and title=:title limit 1", array(":goodsid" => $goodsid, ":title" => $p['title']));
			$paramid = 0;
			$d = array("uniacid" => $_W['uniacid'], "goodsid" => $goodsid, "title" => $p['title'], "value" => $p['value'], "displayorder" => $displayorder);
			if (empty($oldp)) {
				pdo_insert("ewei_dshop_goods_param", $d);
				$paramid = pdo_insertid();
			} else {
				pdo_update('ewei_dshop_goods_param', $d, array("id" => $oldp['id']));
				$paramid = $oldp['id'];
			}
			$paramids[] = $paramid;
			$displayorder++;
		...
		if (count($paramids) > 0) {
			pdo_query("delete from " . tablename('ewei_dshop_goods_param') . " where goodsid=:goodsid and id not in (" . implode(",", $paramids) . ")", array(":goodsid" => $goodsid));
		} else {
			pdo_query('delete from ' . tablename('ewei_dshop_goods_param') . ' where goodsid=:goodsid ', array(':goodsid' => $goodsid));
		}
		$specs = $item['specs'];
		$specids = array();
		$displayorder = 0;
		...
		foreach ($specs as $spec) {
			$oldspec = pdo_fetch("select * from " . tablename('ewei_dshop_goods_spec') . " where goodsid=:goodsid and propId=:propId limit 1", array(":goodsid" => $goodsid, ":propId" => $spec['propId']));
			$specid = 0;
			$d_spec = array("uniacid" => $_W['uniacid'], "goodsid" => $goodsid, "title" => $spec['title'], "displayorder" => $displayorder, "propId" => $spec['propId']);
			if (empty($oldspec)) {
				pdo_insert("ewei_dshop_goods_spec", $d_spec);
				$specid = pdo_insertid();
			} else {
				pdo_update('ewei_dshop_goods_spec', $d_spec, array("id" => $oldspec['id']));
				$specid = $oldspec['id'];
			}
			$d_spec['id'] = $specid;
			$specids[] = $specid;
			...
			foreach ($spec_items as $spec_item) {
				...
				$oldspecitem = pdo_fetch("select * from " . tablename('ewei_dshop_goods_spec_item') . " where specid=:specid and valueId=:valueId limit 1", array(":specid" => $specid, ":valueId" => $spec_item['valueId']));
				$spec_item_id = 0;
				if (empty($oldspecitem)) {
					pdo_insert("ewei_dshop_goods_spec_item", $d);
					$spec_item_id = pdo_insertid();
				} else {
					pdo_update('ewei_dshop_goods_spec_item', $d, array("id" => $oldspecitem['id']));
			...
			if (count($spec_itemids) > 0) {
				pdo_query("delete from " . tablename('ewei_dshop_goods_spec_item') . " where specid=:specid and id not in (" . implode(",", $spec_itemids) . ")", array(":specid" => $specid));
			} else {
				pdo_query('delete from ' . tablename('ewei_dshop_goods_spec_item') . ' where specid=:specid ', array(':specid' => $specid));
			}
			pdo_update('ewei_dshop_goods_spec', array('content' => serialize($spec_itemids)), array("id" => $oldspec['id']));
		}
		if (count($specids) > 0) {
			pdo_query("delete from " . tablename('ewei_dshop_goods_spec') . " where goodsid=:goodsid and id not in (" . implode(",", $specids) . ")", array(":goodsid" => $goodsid));
		} else {
			pdo_query('delete from ' . tablename('ewei_dshop_goods_spec') . ' where goodsid=:goodsid ', array(':goodsid' => $goodsid));
		}
		$minprice = 0;
		...
		foreach ($options as $o) {
			...
			$oldoption = pdo_fetch("select * from " . tablename('ewei_dshop_goods_option') . " where goodsid=:goodsid and skuId=:skuId limit 1", array(":goodsid" => $goodsid, ":skuId" => $o['skuId']));
			$option_id = 0;
			if (empty($oldoption)) {
				pdo_insert("ewei_dshop_goods_option", $do);
				$option_id = pdo_insertid();
			} else {
				pdo_update('ewei_dshop_goods_option', $do, array("id" => $oldoption['id']));
		...
		if (count($optionids) > 0) {
			pdo_query("delete from " . tablename('ewei_dshop_goods_option') . " where goodsid=:goodsid and id not in (" . implode(",", $optionids) . ")", array(":goodsid" => $goodsid));
		} else {
			pdo_query('delete from ' . tablename('ewei_dshop_goods_option') . ' where goodsid=:goodsid ', array(':goodsid' => $goodsid));
		}
		...
		pdo_update('ewei_dshop_goods', $d, array("id" => $goodsid));
	...
	function saveToLocal($url)
	{
		...
		$path = "images/ewei_dshop/" . $_W['uniacid'] . "/" . date('Y') . "/" . date('m') . "/";
		load()->func('file');
