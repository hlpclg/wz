...
if (!class_exists('PermModel')) {
	class PermModel extends PluginModel
	{
		...
		private function check($permtype = '')
		{
			...
			$user = pdo_fetch('select u.status as userstatus,r.status as rolestatus,u.perms as userperms,r.perms as roleperms from ' . tablename('ewei_shop_perm_user') . ' u ' . ' left join ' . tablename('ewei_shop_perm_role') . ' r on u.roleid = r.id ' . ' where uid=:uid limit 1 ', array(':uid' => $uid));
