...
if ($operation == 'display') {
    ...
    $id           = intval($_GPC['id']);
    $item         = pdo_fetch("SELECT * FROM " . tablename('ewei_shop_perm_plugin') . " WHERE id =:id limit 1", array(
        ':id' => $id
    ));
    ...
    if (checksubmit('submit')) {
        ...
        if (!empty($id)) {
            pdo_update('ewei_shop_perm_plugin', $data, array(
                'id' => $id
            ));
        } else {
			
            if (empty($data['type'])) {
                $usercount = pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_perm_plugin') . ' where uid=:uid limit 1', array(
                    ':uid' => $data['uid']
                ));
                if ($usercount > 0) {
                ...
                $wechatcount = pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_perm_plugin') . ' where acid=:acid limit 1', array(
                    ':acid' => $data['acid']
                ));
                if ($wechatcount > 0) {
    ...
    pdo_delete('ewei_shop_perm_plugin', array(
        'id' => $id
    ));
