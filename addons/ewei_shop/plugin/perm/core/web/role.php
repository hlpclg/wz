...
if ($operation == 'display') {
    ...
    foreach ($list as &$row) {
        $row['usercount'] = pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_perm_user') . ' where roleid=:roleid limit 1', array(
            ':roleid' => $row['id']
        ));
    }
    unset($row);
    ...
    $item       = pdo_fetch("SELECT * FROM " . tablename('ewei_shop_perm_role') . " WHERE id =:id and deleted=0 and uniacid=:uniacid limit 1", array(
        ':uniacid' => $_W['uniacid'],
        ':id' => $id
    ));
    ...
    if (checksubmit('submit')) {
        ...
        if (!empty($id)) {
            pdo_update('ewei_shop_perm_role', $data, array(
                'id' => $id,
                'uniacid' => $_W['uniacid']
            ));
    ...
    pdo_update('ewei_shop_perm_role', array(
        'deleted' => 1
    ), array(
        'id' => $id,
        'uniacid' => $_W['uniacid']
