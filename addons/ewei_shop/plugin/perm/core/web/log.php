...
if ($operation == 'display') {
    ...
    if (checksubmit('clearlog')) {
        ...
        pdo_query('delete from ' . tablename('ewei_shop_perm_log') . ' where  uniacid=:uniacid and  createtime >=:starttime  AND createtime <=:endtime', array(
            ':uniacid' => $_W['uniacid'],
            ':starttime' => $starttime,
            ':endtime' => $endtime
        ));
    ...
    pdo_delete('ewei_shop_perm_log', array(
        'id' => $id
    ));
