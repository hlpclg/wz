...
if ($operation == 'addtype') {
    ...
    ca('tmessage.view');
    $list = pdo_fetchall('SELECT * FROM ' . tablename('ewei_shop_member_message_template') . ' WHERE uniacid=:uniacid order by id asc', array(
        ':uniacid' => $_W['uniacid']
    ));
    ...
    if (!empty($_GPC['id'])) {
        $list = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_member_message_template') . ' WHERE id=:id and uniacid=:uniacid ', array(
            ':id' => $_GPC['id'],
            ':uniacid' => $_W['uniacid']
        ));
    ...
    if ($_W['ispost']) {
        ...
        if (empty($id)) {
            ...
            pdo_update('ewei_shop_member_message_template', $insert, array(
                'id' => $id
            ));
        }
    ...
    ca('tmessage.delete');
    $id = intval($_GPC['id']);
    pdo_delete('ewei_shop_member_message_template', array(
        'id' => $id,
        'uniacid' => $_W['uniacid']
    ));
    ...
    ca('tmessage.send');
    $id   = intval($_GPC['id']);
    $send = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_member_message_template') . ' WHERE id=:id and uniacid=:uniacid ', array(
        ':id' => $id,
        ':uniacid' => $_W['uniacid']
    ));
    if (empty($send)) {
    ...
    $value1 = $_GPC['value1'];
    $tpid1  = $_GPC['tpid'];
    pdo_update('ewei_shop_member_message_template', array(
    ...
    if ($class1 == 1) {
        ...
        if (!empty($value1)) {
            $levelname = pdo_fetchcolumn('select levelname from ' . tablename('ewei_shop_member_level') . ' where id=:id limit 1', array(
                ':id' => $value1
            ));
        } else {
            $levelname = "全部";
        ...
        $member = pdo_fetchall("SELECT openid FROM " . tablename('ewei_shop_member') . " WHERE uniacid = '{$_W['uniacid']}'" . $where, array(), 'openid');
        if (!empty($value1)) {
            $groupname = pdo_fetchcolumn('select groupname from ' . tablename('ewei_shop_member_group') . ' where id=:id limit 1', array(
                ':id' => $value1
            ));
        } else {
            $groupname = "全部分组";
        ...
        if (!empty($value1)) {
            $levelname = pdo_fetchcolumn('select levelname from ' . tablename('ewei_shop_commission_level') . ' where id=:id limit 1', array(
                ':id' => $value1
            ));
        } else {
            $levelname = "全部";
    ...
    $id       = intval($_GPC['id']);
    $template = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_member_message_template') . ' WHERE id=:id and uniacid=:uniacid ', array(
        ':id' => $id,
        ':uniacid' => $_W['uniacid']
    ));
    if (empty($template)) {
