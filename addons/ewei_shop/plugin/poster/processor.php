...
class PosterProcessor extends PluginProcessor
{
	...
	private function responseText($obj)
	{
		...
		$timeout = 4;
		load()->func('communication');
		$resp = $_W['siteroot'] . 'app/index.php?i=' . $_W['uniacid'] . '&c=entry&m=ewei_shop&do=plugin&p=poster&method=build&timestamp=' . time();
    ...
    private function responseSubscribe($obj)
    {
        ...
        if (empty($log) && $openid != $qr['openid']) {
			...
			if (!empty($_var_27)) {
				pdo_update('ewei_shop_poster_log', $_var_27, array('id' => $log['id']));
			}
		}
					...
					if ($qrmember['isagent'] == 1 && $qrmember['status'] == 1) {
						if (!empty($poster['bedown'])) {
							...
							if (!empty($poster['beagent'])) {
								...
								pdo_update('ewei_shop_member', array('isagent' => 1, 'status' => $status, 'agenttime' => $time), array('id' => $member['id']));
								if ($status == 1) {
