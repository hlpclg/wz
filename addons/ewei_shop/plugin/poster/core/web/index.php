...
if ($operation == 'display') {
    ...
    $item = pdo_fetch("SELECT * FROM " . tablename('ewei_shop_poster') . " WHERE id =:id and uniacid=:uniacid limit 1", array(
        ':id' => $id,
        ':uniacid' => $_W['uniacid']
    ));
    if (!empty($item)) {
    ...
    if (checksubmit('submit')) {
        ...
        if (!empty($id)) {
            pdo_update('ewei_shop_poster', $data, array(
                'id' => $id,
                'uniacid' => $_W['uniacid']
            ));
        ...
        $rule = pdo_fetch("select * from " . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name  limit 1', array(
            ':uniacid' => $_W['uniacid'],
            ':module' => 'ewei_shop',
            ':name' => "ewei_shop:poster:" . $data['type']
        ));
        if (empty($rule)) {
            $rule_data = array(
                'uniacid' => $_W['uniacid'],
    ...
    pdo_delete('ewei_shop_poster', array(
        'id' => $id,
        'uniacid' => $_W['uniacid']
    ));
    pdo_delete('ewei_shop_poster_log', array(
        'posterid' => $id,
        'uniacid' => $_W['uniacid']
    ));
