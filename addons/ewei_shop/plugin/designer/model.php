...
if (!class_exists('DesignerModel')) {
    class DesignerModel extends PluginModel
    {
        ...
        {
            global $_W, $_GPC;
            $page = pdo_fetch("SELECT * FROM " . tablename('ewei_shop_designer') . " WHERE uniacid= :uniacid and pagetype=:type and setdefault=:default", array(
                ':uniacid' => $_W['uniacid'],
                ':type' => $type,
                ':default' => '1'
            ));
        ...
        public function getData($page)
        {
            ...
            if (count($arr) > 0) {
                $goodinfos = pdo_fetchall("SELECT id,title,productprice,marketprice,thumb,sales,unit FROM " . tablename('ewei_shop_goods') . " WHERE id in ( " . implode(',', $arr) . ") and uniacid= :uniacid ", array(
                    ':uniacid' => $_W['uniacid']
                ), 'id');
        ...
        public function getGuide($system, $pageinfo)
        {
            ...
            if ($guide['followed'] != '1') {
                ...
                if (!empty($_GPC['mid'])) {
                    $guide['member1'] = pdo_fetch("SELECT id,nickname,openid,avatar FROM " . tablename('ewei_shop_member') . " WHERE id=:mid and uniacid= :uniacid limit 1 ", array(
                        ':uniacid' => $_W['uniacid'],
                        ':mid' => $_GPC['mid']
                    ));
                    $guide['member2'] = pdo_fetch("SELECT id,nickname,openid FROM " . tablename('ewei_shop_member') . " WHERE openid=:openid and uniacid= :uniacid limit 1 ", array(
                        ':uniacid' => $_W['uniacid'],
        ...
        {
            global $_W;
            return pdo_fetchcolumn('select id from ' . tablename('ewei_shop_designer_menu') . ' where isdefault=1 and uniacid=:uniacid limit 1', array(
                ':uniacid' => $_W['uniacid']
            ));
        }
        ...
        {
            global $_W;
            return pdo_fetch('select * from ' . tablename('ewei_shop_designer_menu') . ' where isdefault=1 and uniacid=:uniacid limit 1', array(
                ':uniacid' => $_W['uniacid']
            ));
        }
