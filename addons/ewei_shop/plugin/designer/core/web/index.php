...
if ($op == 'display') {
    ...
    $pages    = pdo_fetchall("SELECT * FROM " . tablename('ewei_shop_designer') . " WHERE uniacid= :uniacid and pagename LIKE :name " . "ORDER BY savetime DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize, array(
        ':uniacid' => $_W['uniacid'],
        ':name' => "%{$kw}%"
    ));
    $pagesnum = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename('ewei_shop_designer') . " WHERE uniacid= :uniacid " . "ORDER BY savetime DESC ", array(
        ':uniacid' => $_W['uniacid']
    ));
    $total    = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename('ewei_shop_designer') . " WHERE uniacid= :uniacid and pagename LIKE :name " . "ORDER BY savetime DESC ", array(
        ':uniacid' => $_W['uniacid'],
        ':name' => "%{$kw}%"
    ));
    $pager    = pagination($total, $pindex, $psize);
} elseif ($op == 'post') {
    $menus     = pdo_fetchall("SELECT id,menuname,isdefault FROM " . tablename('ewei_shop_designer_menu') . " WHERE uniacid= :uniacid  ", array(
        ':uniacid' => $_W['uniacid']
    ));
    $pages     = pdo_fetchall("SELECT id,pagename,pagetype,setdefault FROM " . tablename('ewei_shop_designer') . " WHERE uniacid= :uniacid  ", array(
        ':uniacid' => $_W['uniacid']
    ));
    $categorys = pdo_fetchall("SELECT id,name,parentid FROM " . tablename('ewei_shop_category') . " WHERE enabled=:enabled and uniacid= :uniacid  ", array(
        ':uniacid' => $_W['uniacid'],
        ':enabled' => '1'
    ));
    if (!empty($pageid)) {
        ca('designer.page.edit');
        $datas = pdo_fetch("SELECT * FROM " . tablename('ewei_shop_designer') . " WHERE uniacid= :uniacid and id=:id", array(
            ':uniacid' => $_W['uniacid'],
            ':id' => $pageid
        ));
            ...
            foreach ($data as $i1 => &$dd) {
                if ($dd['temp'] == 'goods') {
                    foreach ($dd['data'] as $i2 => &$ddd) {
                        $goodinfo = pdo_fetchall("SELECT id,title,productprice,marketprice,thumb FROM " . tablename('ewei_shop_goods') . " WHERE uniacid= :uniacid and id=:goodid", array(
                            ':uniacid' => $_W['uniacid'],
                            ':goodid' => $ddd['goodid']
                        ));
    ...
    if ($_W['ispost']) {
        if ($apido == 'savepage') {
            ...
            if (empty($id)) {
                ...
                pdo_update('ewei_shop_designer', $insert, array(
                    'id' => $id
                ));
            ...
            if (empty($pageid)) {
                ...
                $page = pdo_fetch("SELECT * FROM " . tablename('ewei_shop_designer') . " WHERE uniacid= :uniacid and id=:id", array(
                    ':uniacid' => $_W['uniacid'],
                    ':id' => $pageid
                ));
                if (empty($page)) {
                    ...
                    $do = pdo_delete('ewei_shop_designer', array(
                        'id' => $pageid
                    ));
                    if ($do) {
                        $rule = pdo_fetch("select * from " . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name  limit 1', array(
                            ':uniacid' => $_W['uniacid'],
                            ':module' => 'ewei_shop',
                            ':name' => "ewei_shop:designer:" . $pageid
                        ));
                        if (!empty($rule)) {
            ...
            $kw    = $_GPC['kw'];
            $goods = pdo_fetchall("SELECT id,title,productprice,marketprice,thumb,sales,unit FROM " . tablename('ewei_shop_goods') . " WHERE uniacid= :uniacid and status=:status and deleted=0 AND title LIKE :title ", array(
                ':title' => "%{$kw}%",
                ':uniacid' => $_W['uniacid'],
                ':status' => '1'
            ));
            ...
            $type = $_GPC['type'];
            if ($do == 'on') {
                $pages = pdo_fetch("SELECT * FROM " . tablename('ewei_shop_designer') . " WHERE pagetype=:pagetype and setdefault=:setdefault and uniacid=:uniacid ", array(
                    ':pagetype' => $type,
                    ':setdefault' => '1',
                    ':uniacid' => $_W['uniacid']
                ));
                if (!empty($pages)) {
                    ...
                    pdo_update('ewei_shop_designer', $array, array(
                        'id' => $pages['id']
                    ));
                }
                $array  = array(
                ...
                $action = pdo_update('ewei_shop_designer', $array, array(
                    'id' => $id
                ));
                if ($action) {
                    $json = array(
                ...
                $pages = pdo_fetch("SELECT * FROM " . tablename('ewei_shop_designer') . " WHERE  id=:id and uniacid=:uniacid ", array(
                    ':id' => $id,
                    ':uniacid' => $_W['uniacid']
                ));
            ...
            $kw   = $_GPC['kw'];
            if ($type == 'notice') {
                $notices = pdo_fetchall("select * from " . tablename('ewei_shop_notice') . ' where title LIKE :title and status=:status and uniacid=:uniacid ', array(
                    ':uniacid' => $_W['uniacid'],
                    ':status' => '1',
                    ':title' => "%{$kw}%"
                ));
                ...
                $goods = pdo_fetchall("select title,id,thumb,marketprice,productprice from " . tablename('ewei_shop_goods') . ' where title LIKE :title and status=1 and deleted=0 and uniacid=:uniacid ', array(
                    ':uniacid' => $_W['uniacid'],
                    ':title' => "%{$kw}%"
                ));
                $goods = set_medias($goods, 'thumb');
                ...
                $articles = pdo_fetchall("select id,article_title from " . tablename('ewei_shop_article') . ' where article_title LIKE :title and article_state=1 and uniacid=:uniacid ', array(
                    ':uniacid' => $_W['uniacid'],
                    ':title' => "%{$kw}%"
                ));
