...
//-----------------------------------web端
		...
		if($op=='recharge'){
			...
			if($_GPC['opp']=='recharged'){
				...
				if(empty($temp)){
					...
					message('充值成功！', $this->createWebUrl('fansmanager', array('opp'=>'', 'op'=>'recharge', 'id'=>$_GPC['id'])), 'success');
				}
			}
			$user = pdo_fetch("select * from ".tablename('eso_sale_member'). " where id = ".$id);
	...
	public function doMobilelist() {
		...
		$logo = $cfg['logo'];
		$ydyy = $cfg['ydyy'];
		$description = strip_tags($cfg['description']);
	...
	public function doMobilelist2() {
		...
		$sorturl = $this->mturl('list2', array("keyword" => $_GPC['keyword'], "pcate" => $_GPC['pcate'], "ccate" => $_GPC['ccate']));
		if (!empty($_GPC['isnew'])) {
	...
	public function setOrderCredit($orderid, $add = true) {
		...
		$ordergoods = pdo_fetchall("SELECT goodsid, total FROM " . tablename('eso_sale_order_goods') . " WHERE orderid = '{$orderid}'", array(), 'goodsid');
		if (!empty($ordergoods)) {
	...
	public function doMobilePay() {
		...
		if ($order['status'] != '0') {
			message('抱歉，您的订单已经付款或是被关闭，请重新进入付款！', $this->mturl('myorder'), 'error');
		}
		if (checksubmit('codsubmit')) {
			...
			message('订单提交成功，请您收到货时付款！', $this->mturl('myorder'), 'success');
		}
	...
	public function payResult($params) {
		...
		if ($params['from'] == 'return') {
			//邮件提醒
			if (!empty($this->module['config']['noticeemail'])) {
				$order = pdo_fetch("SELECT price, from_user FROM " . tablename('eso_sale_order') . " WHERE id = '{$params['tid']}'");
			...
			if ($params['type'] == 'credit2') {
				message('支付成功！', $this->mturl('myorder'), 'success');
			} else {
				message('支付成功！', '../../app/' . $this->mturl('myorder'), 'success');
			}
		}
	}
	...
	public function doMobileCredit() {
		...
		if (!empty($_GPC['award_id']))
		{
			...
			if ($fans['credit1'] >= $award_info['credit_cost'] && $award_info['amount'] > 0)
			{
				...
				message('积分兑换成功！', create_url('mobile/module/mycredit', array('uniacid' => $_W['uniacid'], 'name' => 'eso_sale', 'do' => 'mycredit','op' => 'display')), 'success');
			}
			else
			{
				message('积分不足或商品已经兑空，请重新选择商品！<br>当前商品所需积分:'.$award_info['credit_cost'].'<br>您的积分:'.$fans['credit1']
					...
					create_url('mobile/module/award', array('uniacid' => $_W['uniacid'], 'name' => 'eso_sale')), 'error');
			}
		}
		else
		{
			message('请选择要兑换的商品！', create_url('mobile/module/award', array('uniacid' => $_W['uniacid'], 'name' => 'eso_sale')), 'error');
		}
	}
...
*/
		...
		$total = pdo_fetchcolumn("SELECT COUNT(*) FROM ".tablename('core_paylog')." WHERE  module='eso_sale' AND type='zhifu' AND openid='".$from_user."' AND `uniacid` = ".$_W['uniacid']);
		$pager = pagination($total, $pindex, $psize);
		$list = pdo_fetchall("SELECT * FROM ".tablename('core_paylog')." WHERE module='eso_sale' AND type='zhifu' AND openid='".$from_user."' AND uniacid=".$_W['uniacid']." ORDER BY plid DESC LIMIT " . ($pindex - 1) * $psize . "," . $psize);
	...
	public function doWebZhifu() {
		...
		if($op=='post'){
			if(empty($_GPC['from_user'])){
				message('请选择会员！', create_url('site/entry', array('do' => 'zhifu','op'=>'list', 'm' => 'eso_sale','uniacid'=>$_W['uniacid'])), 'success');
			}
			if(checksubmit()){
			...
			$total = pdo_fetchcolumn("SELECT COUNT(*) FROM ".tablename('core_paylog')." WHERE  module='eso_sale' AND type='zhifu' AND openid='".$_GPC['from_user']."' AND `uniacid` = ".$_W['uniacid']);
			$pager = pagination($total, $pindex, $psize);
			$list = pdo_fetchall("SELECT * FROM ".tablename('core_paylog')." WHERE module='eso_sale' AND type='zhifu' AND openid='".$_GPC['from_user']."' AND uniacid=".$_W['uniacid']." ORDER BY plid DESC LIMIT " . ($pindex - 1) * $psize . "," . $psize);
	...
	private function get_weixin_token() {
		...
		$acc = WeAccount::create($accountdata['acid']);
		$access_token = $acc->fetchAccountInfo();
