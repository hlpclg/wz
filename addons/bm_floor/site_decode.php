...
class bm_floorModuleSite extends WeModuleSite {
	...
	public function doMobileshow(){
		...
		$orderNum =  pdo_fetchcolumn("select COUNT(*) from ".tablename('bm_floor_member')." where rid = '{$rid}' and weid='{$weid}' and  share_point>(SELECT share_point from ".tablename('bm_floor_member')." where from_user='".$fromuser."' and rid = '{$rid}' and weid='{$weid}') order by share_point desc");
	...
	public function doMobileajaxGetUser(){
		...
		$pan = pdo_fetch("SELECT * FROM ".tablename('bm_floor_member')." WHERE rid = '{$rid}' and from_user='{$fromuser}' LIMIT 1");
		if(empty($pan)){
