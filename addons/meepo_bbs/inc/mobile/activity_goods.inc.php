...
if($do == 'post') {
	...
	message("兑换成功,您消费了 {$goods['credit']} {$creditnames[$goods['credittype']]},现在去完善订单信息", $this->createMobileUrl('activity_goods', array('op'=>'deliver','tid' => $ret)));
}
if($do == 'deliver') {
	load()->func('tpl');
	$tid = intval($_GPC['tid']);
	...
	if(checksubmit('submit')) {
		...
		message('收货人信息更新成功', $this->createMobileUrl('activity_goods',array('op'=>'mine')));
	}
}
if($do == 'mine') {
	$psize = 10;
...
if($do == 'confirm') {
	...
	message('确认收货成功', $this->createMobileUrl('activity_goods',array('op'=>'mine','status'=>2)), 'success');
}

include $this->template($tempalte.'/templates/activity/goods');