...
$reply = pdo_fetch("SELECT isrealname,ismobile,isqq,isemail,isaddress,isgender,istelephone,isidcard,iscompany,isoccupation,isposition,isfansname FROM " . tablename('stonefish_scratch_reply') . " WHERE rid = :rid ORDER BY `id` DESC", array(':rid' => $rid));
...
if($fans=='branch'){
	...
	foreach ($list as &$row) {
		...
		$fansID = pdo_fetchcolumn("SELECT fansID FROM " . tablename('stonefish_scratch_fans') . "  WHERE mobile = :mobile and rid=:rid and uniacid=:uniacid", array(':mobile' => $row['mobile'],':rid' => $rid,':uniacid' => $_W['uniacid']));
		$row['awardinfo']='';
		$awards = pdo_fetchall("SELECT name FROM " . tablename('stonefish_scratch_award') . " WHERE rid = :rid and fansID=:fansID", array(':rid' => $rid,':fansID' => $fansID));
		if(!empty($awards)){
	...
	foreach ($list as &$lists) {
		$lists['awardinfo']='';
		$awards = pdo_fetchall("SELECT name FROM " . tablename('stonefish_scratch_award') . " WHERE rid = :rid and fansID=:fansID", array(':rid' => $rid,':fansID' => $lists['fansID']));
		if(!empty($awards)){
			foreach ($awards as &$awardid) {
    ...
    $list = pdo_fetchall("SELECT a.*,b.realname,b.mobile,b.qq,b.email,b.address,b.gender,b.telephone,b.idcard,b.company,b.occupation,b.position FROM ".tablename('stonefish_scratch_award')." as a  left join ".tablename('stonefish_scratch_fans')." 
